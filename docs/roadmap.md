...
the backup review reaches the final page and is approved. The desktop
`backup-source` harness now exposes that ceremony for future Pi output-adapter
...
Status: Raspberry now consumes shared source public-key proof vectors for
NIP-06 and NIP-19 `nsec` session sources. The package derives the reviewed
public key from the RAM-only source instead of trusting descriptor metadata or
