...
def _require_template(template: dict[str, Any]) -> None:
    ...
    if template["created_at"] > NOSTRSEAL_V0_LIMITS["max_safe_integer"]:
    ...
    if template["kind"] > NOSTRSEAL_V0_LIMITS["max_safe_integer"]:
        raise ValueError("event_template kind exceeds max_safe_integer")
    if not isinstance(template["content"], str):
        raise ValueError("content must be a string")
    if utf8_size(template["content"]) > NOSTRSEAL_V0_LIMITS["max_content_utf8_bytes"]:
    ...
    if len(template["tags"]) > NOSTRSEAL_V0_LIMITS["max_tag_count"]:
    ...
    for tag_index, tag in enumerate(template["tags"]):
        ...
        if len(tag) > NOSTRSEAL_V0_LIMITS["max_tag_fields_per_tag"]:
        ...
        for field_index, item in enumerate(tag):
            ...
            if item_bytes > NOSTRSEAL_V0_LIMITS["max_tag_field_utf8_bytes"]:
                raise ValueError("event_template tag field exceeds max_tag_field_utf8_bytes")
    if total_tag_bytes > NOSTRSEAL_V0_LIMITS["max_total_tag_utf8_bytes"]:
